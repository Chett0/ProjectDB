...
class Route(db.Model):
    ...
    departure_airport_id : Mapped[int] = mapped_column(db.ForeignKey("airports.id", ondelete='CASCADE'))
    departure_airport : Mapped["Airport"] = relationship(
        "Airport", 
        back_populates="departure_route",
        foreign_keys=[departure_airport_id]
    )
    arrival_airport_id : Mapped[int] = mapped_column(db.ForeignKey("airports.id", ondelete='CASCADE'))
